...
public class FlarePistolItem extends Item {
   ...
   public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
      ...
      } else if (offhandStack.getItem() instanceof RaidFlareItem flareItem) {
         if (!level.isClientSide) {
            if (level instanceof ServerLevel serverLevel && RaidManager.hasActiveRaidInDimension(serverLevel)) {
            ...
            double speed = 2.5;
            flare.setDeltaMovement(lookAngle.x * speed, lookAngle.y * speed, lookAngle.z * speed);
            level.addFreshEntity(flare);
            level.playSound(null, player.getX(), player.getY(), player.getZ(), SoundEvents.FIREWORK_ROCKET_LAUNCH, SoundSource.PLAYERS, 1.2F, 0.9F);
   ...
   public void appendHoverText(ItemStack stack, Item.TooltipContext level, List<Component> tooltip, TooltipFlag flag) {
      ...
      tooltip.add(Component.translatable("item.scguns.flare_pistol.usage").withStyle(new ChatFormatting[]{ChatFormatting.DARK_GRAY, ChatFormatting.ITALIC}));
   }
}
