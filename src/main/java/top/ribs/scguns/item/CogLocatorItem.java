...
public class CogLocatorItem extends Item {
   ...
   public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
      ...
      if (!level.isClientSide) {
         ...
         if (structurePos != null) {
            ...
            level.playSound(null, player.getX(), player.getY(), player.getZ(), SoundEvents.BEACON_ACTIVATE, SoundSource.PLAYERS, 0.6F, 1.2F);
            itemStack.hurtAndBreak(1, player, net.minecraft.world.entity.LivingEntity.getSlotForHand(player.getUsedItemHand()));
            return InteractionResultHolder.success(itemStack);
         } else {
            level.playSound(null, player.getX(), player.getY(), player.getZ(), SoundEvents.BEACON_DEACTIVATE, SoundSource.PLAYERS, 0.5F, 0.8F);
   ...
   private BlockPos findNearestCogChambers(ServerLevel level, BlockPos playerPos) {
      ...
      ResourceKey<Structure> cogChambersKey = ResourceKey.create(Registries.STRUCTURE, ResourceLocation.fromNamespaceAndPath("scguns", "cog_chambers"));
      Optional<Reference<Structure>> cogChambers = structureRegistry.getHolder(cogChambersKey);
      if (cogChambers.isPresent()) {
         HolderSet<Structure> holderSet = HolderSet.direct(new Holder[]{(Holder)cogChambers.get()});
         Pair<BlockPos, Holder<Structure>> structurePos = level.getChunkSource().getGenerator().findNearestMapStructure(level, holderSet, playerPos, 100, false);
         if (structurePos != null) {
   ...
   private void createPlasmaRingLine(ServerLevel level, Vec3 start, Vec3 direction) {
      ...
      for (int i = 0; i < ringCount; i++) {
         ...
         Vec3 ringPos = start.add(
            direction.x * lineLength * progress, direction.y * lineLength * progress + 1.5, direction.z * lineLength * progress
         );
         ...
         level.sendParticles(
            (SimpleParticleType)ModParticleTypes.PLASMA_RING.get(),
            ringPos.x + offsetX,
            ringPos.y + offsetY,
            ringPos.z + offsetZ,
            1,
