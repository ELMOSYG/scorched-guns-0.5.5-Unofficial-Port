...
public class S2CMessageMuzzleFlash {
   ...
   public S2CMessageMuzzleFlash decode(FriendlyByteBuf buffer) {
      return new S2CMessageMuzzleFlash(new Vec3(buffer.readDouble(), buffer.readDouble(), buffer.readDouble()), buffer.readFloat(), buffer.readFloat());
   }
   ...
   public void handle(S2CMessageMuzzleFlash message, MessageContext context) {
      context.execute(
         () -> {
            ...
            if (level != null) {
               level.addParticle(
                  (ParticleOptions)ModParticleTypes.TURRET_MUZZLE_FLASH.get(),
                  message.position.x,
                  message.position.y,
                  message.position.z,
                  0.0,
