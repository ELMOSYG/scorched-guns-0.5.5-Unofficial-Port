...
public enum Easings implements StringRepresentable {
   ...
   EASE_IN_CUBIC("ease_in_cubic", t -> (float)Math.pow((double)t.floatValue(), 3.0)),
   EASE_IN_CIRC("ease_in_circ", t -> 1.0F - Mth.sqrt(1.0F - Mth.square(t))),
   EASE_OUT_QUAD("ease_out_quad", t -> 1.0F - Mth.square(1.0F - t)),
   EASE_OUT_CUBIC("ease_out_cubic", t -> 1.0F - (float)Math.pow((double)(1.0F - t), 3.0)),
   ...
   EASE_IN_OUT_CUBIC(
      "ease_in_out_cubic", t -> t < 0.5F ? 4.0F * (float)Math.pow((double)t.floatValue(), 3.0) : 1.0F - (float)Math.pow((double)(-2.0F * t + 2.0F), 3.0) / 2.0F
   ),
   EASE_IN_OUT_CIRC(
      "ease_in_out_circ",
      t -> t < 0.5F ? (1.0F - Mth.sqrt(1.0F - Mth.square(2.0F * t))) / 2.0F : (Mth.sqrt(1.0F - Mth.square(-2.0F * t + 2.0F)) + 1.0F) / 2.0F
   ),
   HALF_EASE_IN("half_ease_in", t -> t > 0.5F ? EASE_IN_QUAD.apply((t - 0.5F) / 0.5F) : 0.0F),
   ZERO("zero", t -> 0.0F);

   private static final Map<String, Easings> BY_NAME = Arrays.stream(values()).collect(Collectors.toMap(easing -> easing.name, easing -> (Easings)easing));
   ...
   public double apply(double time) {
      return (double)this.function.apply(Double.valueOf(time).floatValue()).floatValue();
   }
