...
public class RenderUtil {
   ...
   public static void renderModel(
      ItemStack stack, ItemDisplayContext display, PoseStack poseStack, MultiBufferSource buffer, int light, int overlay, @Nullable LivingEntity entity
   ) {
   ...
   public static void renderModel(
      BakedModel model, ItemDisplayContext display, ItemStack stack, PoseStack poseStack, MultiBufferSource buffer, int light, int overlay
   ) {
      ...
      if (!stack.isEmpty()) {
         ...
         if (!model.isCustomRenderer() && (stack.getItem() != Items.TRIDENT || flag)) {
            ...
            if (stack.getItem() == Items.COMPASS && stack.hasFoil()) {
               // 1.21 dropped getCompassFoilBufferDirect and works on a copy of the
               // top pose instead of pushing/popping the stack around it.
               Pose entry = poseStack.last().copy();
   ...
   public static void renderModelWithTransforms(
      ItemStack child, ItemStack parent, ItemDisplayContext display, PoseStack poseStack, MultiBufferSource buffer, int light, int overlay
   ) {
      poseStack.pushPose();
      BakedModel model = Minecraft.getInstance().getItemRenderer().getItemModelShaper().getItemModel(child);
   ...
   public static void renderItemWithoutTransforms(
      BakedModel model, ItemStack stack, ItemStack parent, PoseStack poseStack, MultiBufferSource buffer, int light, int overlay
   ) {
   ...
   public static void renderItemWithoutTransforms(
      BakedModel model, ItemStack stack, ItemStack parent, PoseStack poseStack, MultiBufferSource buffer, int light, int overlay, @Nullable Runnable transform
   ) {
      RenderType renderType = getRenderType(stack, false);
      VertexConsumer builder = ItemRenderer.getFoilBuffer(buffer, renderType, true, stack.hasFoil() || parent.hasFoil());
      renderModel(model, stack, parent, transform, poseStack, builder, light, overlay);
   ...
   public static void renderModel(
      BakedModel model, ItemStack stack, ItemStack parent, @Nullable Runnable transform, PoseStack poseStack, VertexConsumer buffer, int light, int overlay
   ) {
      if (transform != null) {
         transform.run();
      }
   ...
   private static void renderQuads(PoseStack poseStack, VertexConsumer buffer, List<BakedQuad> quads, ItemStack stack, ItemStack parent, int light, int overlay) {
      ...
      for (BakedQuad quad : quads) {
         ...
         // 1.21 added the alpha argument (1.20.1 hard-coded it to 1.0F).
         buffer.putBulkData(entry, quad, red, green, blue, 1.0F, light, overlay);
      }
   }
   ...
   public static int getItemStackColor(ItemStack stack, ItemStack parent, int tintIndex) {
      ...
      return color == -1 && !parent.isEmpty() ? getItemStackColor(parent, ItemStack.EMPTY, tintIndex) : color;
   }
   ...
   public static void renderModel(BakedModel model, PoseStack matrixStack, MultiBufferSource buffer, int combinedLight, int combinedOverlay) {
      renderModel(model, ItemDisplayContext.NONE, ItemStack.EMPTY, matrixStack, buffer, combinedLight, combinedOverlay);
   }
}
