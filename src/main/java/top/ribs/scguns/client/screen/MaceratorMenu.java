...
public class MaceratorMenu extends AbstractContainerMenu {
   ...
   public MaceratorMenu(int id, Inventory inv, RegistryFriendlyByteBuf extraData) {
      this(id, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(4));
   }

   public MaceratorMenu(int id, Inventory inv, BlockEntity entity, ContainerData data) {
      super((MenuType)ModMenuTypes.MACERATOR_MENU.get(), id);
      checkContainerSize(inv, 6);
      this.blockEntity = (MaceratorBlockEntity)entity;
   ...
   public boolean stillValid(Player player) {
      return stillValid(ContainerLevelAccess.create(this.level, this.blockEntity.getBlockPos()), player, (Block)ModBlocks.MACERATOR.get());
   }
   ...
   private boolean isInputItem(ItemStack stack) {
      if (this.level == null) {
         ...
         return recipeManager.getAllRecipesFor(MaceratorRecipe.Type.INSTANCE)
            .stream()
   ...
   private boolean isFuel(ItemStack stack) {
      return ScFuels.burnTime(stack) > 0;
   }
