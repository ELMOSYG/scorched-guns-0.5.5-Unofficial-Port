...
public class MechanicalPressScreen extends AbstractContainerScreen<MechanicalPressMenu> {
   private static final ResourceLocation TEXTURE = ResourceLocation.fromNamespaceAndPath("scguns", "textures/gui/mechanical_press_gui.png");
   ...
   private void renderProgressArrow(GuiGraphics guiGraphics, int x, int y) {
      if (((MechanicalPressMenu)this.menu).isCrafting()) {
         int progress = ((MechanicalPressMenu)this.menu).getScaledProgress();
         guiGraphics.blit(TEXTURE, x + 84, y + 25, 176, 14, progress + 1, 16);
      }
   }

   private void renderFuelBar(GuiGraphics guiGraphics, int x, int y) {
      if (((MechanicalPressMenu)this.menu).hasFuel()) {
         int fuel = ((MechanicalPressMenu)this.menu).getScaledFuelProgress();
