...
public class PoweredMaceratorMenu extends AbstractContainerMenu {
   ...
   public PoweredMaceratorMenu(int id, Inventory inv, RegistryFriendlyByteBuf extraData) {
      this(id, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(4));
   }

   public PoweredMaceratorMenu(int id, Inventory inv, BlockEntity entity, ContainerData data) {
      super((MenuType)ModMenuTypes.POWERED_MACERATOR_MENU.get(), id);
      checkContainerSize(inv, 5);
      this.blockEntity = (PoweredMaceratorBlockEntity)entity;
      ...
      Caps.ifPresent(Caps.itemHandler(this.blockEntity, null), handler -> {
         ...
         this.addSlot(new SlotItemHandler(handler, 2, 44, 45));
         this.addSlot(new SlotItemHandler(handler, 3, 62, 45));
   ...
   public boolean stillValid(Player player) {
      return stillValid(ContainerLevelAccess.create(this.level, this.blockEntity.getBlockPos()), player, (Block)ModBlocks.POWERED_MACERATOR.get());
   }
   ...
   private boolean isInputItem(ItemStack stack) {
      if (this.level == null) {
         ...
         return recipeManager.getAllRecipesFor(PoweredMaceratorRecipe.Type.INSTANCE)
            .stream()
