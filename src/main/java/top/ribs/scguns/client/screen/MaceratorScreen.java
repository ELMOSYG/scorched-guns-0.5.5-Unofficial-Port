...
public class MaceratorScreen extends AbstractContainerScreen<MaceratorMenu> {
   private static final ResourceLocation TEXTURE = ResourceLocation.fromNamespaceAndPath("scguns", "textures/gui/macerator_gui.png");
   ...
   private void renderProgressArrow(GuiGraphics guiGraphics, int x, int y) {
      if (((MaceratorMenu)this.menu).isCrafting()) {
         int progress = ((MaceratorMenu)this.menu).getScaledProgress();
         guiGraphics.blit(TEXTURE, x + 84, y + 25, 176, 14, progress + 1, 16);
      }
   }

   private void renderFuelBar(GuiGraphics guiGraphics, int x, int y) {
      if (((MaceratorMenu)this.menu).hasFuel()) {
         int fuel = ((MaceratorMenu)this.menu).getScaledFuelProgress();
