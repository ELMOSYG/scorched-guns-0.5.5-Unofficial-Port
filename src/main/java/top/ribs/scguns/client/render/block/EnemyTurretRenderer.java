...
public class EnemyTurretRenderer implements BlockEntityRenderer<EnemyTurretBlockEntity> {
   public EnemyTurretRenderer(Context context) {
      super();
   }
   ...
   public void render(EnemyTurretBlockEntity turret, float partialTicks, PoseStack matrixStack, MultiBufferSource buffer, int light, int overlay) {
      ...
      this.renderTurretTop(
         turret, matrixStack, buffer, light, overlay, SpecialModels.ENEMY_TURRET_TOP.getModel(), 0.5, 1.0, 0.5, interpolatedYaw, interpolatedPitch
      );
   }

   private void renderTurretTop(
      EnemyTurretBlockEntity turret,
