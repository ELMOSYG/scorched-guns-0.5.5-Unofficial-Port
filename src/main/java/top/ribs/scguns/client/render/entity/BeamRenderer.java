...
public class BeamRenderer {
   ...
   public static void renderBeam(
      ...
      if (!axis.equals(Vec3.ZERO)) {
         pPoseStack.mulPose(
            Axis.of(new Vector3f((float)axis.x, (float)axis.y, (float)axis.z)).rotationDegrees((float)Math.toDegrees((double)angle))
         );
      }

      assert Minecraft.getInstance().level != null;

      renderBeamSegment(pPoseStack, pBuffer, partialTicks, Minecraft.getInstance().level.getGameTime(), 0.0F, length, color);
      pPoseStack.popPose();
   }

   public static Vec3 lerpVec3(float partialTicks, Vec3 lastPos, Vec3 currentPos) {
      double x = Mth.lerp((double)partialTicks, lastPos.x, currentPos.x);
      double y = Mth.lerp((double)partialTicks, lastPos.y, currentPos.y);
      double z = Mth.lerp((double)partialTicks, lastPos.z, currentPos.z);
      return new Vec3(x, y, z);
   }

   public static void renderBeamSegment(
      PoseStack pPoseStack, MultiBufferSource pBufferSource, float pPartialTick, long pGameTime, float pYOffset, float pHeight, float[] pColors
   ) {
      renderBeamPart(pPoseStack, pBufferSource, BEAM_LOCATION, pPartialTick, 1.0F, pGameTime, 0, Math.round(pHeight), pColors, 0.015F, 0.02F);
   }

   public static void renderBeamPart(
      PoseStack pPoseStack,
