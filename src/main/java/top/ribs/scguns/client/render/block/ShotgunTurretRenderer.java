...
public class ShotgunTurretRenderer implements BlockEntityRenderer<ShotgunTurretBlockEntity> {
   public ShotgunTurretRenderer(Context context) {
      super();
   }
   ...
   public void render(ShotgunTurretBlockEntity turret, float partialTicks, PoseStack matrixStack, MultiBufferSource buffer, int light, int overlay) {
      ...
      this.renderTurretTop(
         turret, matrixStack, buffer, light, overlay, SpecialModels.SHOTGUN_TURRET_TOP.getModel(), 0.5, 1.0, 0.5, interpolatedYaw, interpolatedPitch
      );
   }

   private void renderTurretTop(
      ShotgunTurretBlockEntity turret,
