...
public class MiniGun5Pose extends WeaponPose {
   ...
   protected AimPose getUpPose() {
      ...
      pose.getIdle()
         .setRenderYawOffset(35.0F)
         .setItemRotation(new Vector3f(10.0F, 0.0F, 0.0F))
         .setRightArm(
            new LimbPose().setRotationAngleX(-170.0F).setRotationAngleY(-35.0F).setRotationAngleZ(0.0F).setRotationPointY(4.0F).setRotationPointZ(-2.0F)
         )
         .setLeftArm(
            new LimbPose()
               .setRotationAngleX(-130.0F)
               .setRotationAngleY(65.0F)
               .setRotationAngleZ(0.0F)
               .setRotationPointX(3.0F)
   ...
   protected AimPose getForwardPose() {
      ...
      pose.getIdle()
         .setRenderYawOffset(35.0F)
         .setRightArm(
            new LimbPose().setRotationAngleX(-90.0F).setRotationAngleY(-35.0F).setRotationAngleZ(0.0F).setRotationPointY(2.0F).setRotationPointZ(0.0F)
         )
         .setLeftArm(
            new LimbPose()
               .setRotationAngleX(-91.0F)
               .setRotationAngleY(35.0F)
               .setRotationAngleZ(0.0F)
               .setRotationPointX(4.0F)
               .setRotationPointY(2.0F)
               .setRotationPointZ(0.0F)
         );
      return pose;
   }
   ...
   protected AimPose getDownPose() {
      ...
      pose.getIdle()
         .setRenderYawOffset(35.0F)
         .setRightArm(
            new LimbPose().setRotationAngleX(-10.0F).setRotationAngleY(-35.0F).setRotationAngleZ(0.0F).setRotationPointY(2.0F).setRotationPointZ(0.0F)
         )
         .setLeftArm(
            new LimbPose()
               .setRotationAngleX(-10.0F)
               .setRotationAngleY(15.0F)
               .setRotationAngleZ(30.0F)
               .setRotationPointX(4.0F)
               .setRotationPointY(2.0F)
               .setRotationPointZ(0.0F)
         );
   ...
   public void applyPlayerModelRotation(Player player, ModelPart rightArm, ModelPart leftArm, ModelPart head, InteractionHand hand, float aimProgress) {
      if ((Boolean)Config.CLIENT.display.oldAnimations.get()) {
         ...
         mainArm.xRot = (float)Math.toRadians(-90.0);
         mainArm.yRot = (float)Math.toRadians(-35.0) * (right ? 1.0F : -1.0F);
         mainArm.zRot = (float)Math.toRadians(0.0);
         secondaryArm.xRot = (float)Math.toRadians(-91.0);
         secondaryArm.yRot = (float)Math.toRadians(45.0) * (right ? 1.0F : -1.0F);
   ...
   public void applyPlayerPreRender(Player player, InteractionHand hand, float aimProgress, PoseStack poseStack, MultiBufferSource buffer) {
      if ((Boolean)Config.CLIENT.display.oldAnimations.get()) {
         boolean right = Minecraft.getInstance().options.mainHand().get() == HumanoidArm.RIGHT
            ? hand == InteractionHand.MAIN_HAND
            : hand == InteractionHand.OFF_HAND;
         player.yBodyRotO = player.yRotO + 35.0F * (right ? 1.0F : -1.0F);
         player.yBodyRot = player.getYRot() + 35.0F * (right ? 1.0F : -1.0F);
      } else {
   ...
   public void renderFirstPersonArms(
      ...
      if (!(stack.getItem() instanceof AnimatedGunItem)) {
         ...
         poseStack.translate(-0.55 * (double)side, -0.0, -0.7);
