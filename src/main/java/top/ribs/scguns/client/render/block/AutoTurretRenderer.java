...
public class AutoTurretRenderer implements BlockEntityRenderer<AutoTurretBlockEntity> {
   public AutoTurretRenderer(Context context) {
      super();
   }
   ...
   public void render(AutoTurretBlockEntity turret, float partialTicks, PoseStack matrixStack, MultiBufferSource buffer, int light, int overlay) {
      ...
      this.renderTurretTop(
         turret, matrixStack, buffer, light, overlay, SpecialModels.AUTO_TURRET_TOP.getModel(), 0.5, 1.0, 0.5, interpolatedYaw, interpolatedPitch
      );
   }

   private void renderTurretTop(
      AutoTurretBlockEntity turret,
