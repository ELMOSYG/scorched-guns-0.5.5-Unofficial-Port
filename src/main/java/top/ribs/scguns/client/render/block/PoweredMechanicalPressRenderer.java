...
public class PoweredMechanicalPressRenderer implements BlockEntityRenderer<PoweredMechanicalPressBlockEntity> {
   public PoweredMechanicalPressRenderer(Context context) {
      super();
   }

   public void render(
      PoweredMechanicalPressBlockEntity pressEntity, float partialTicks, PoseStack matrixStack, MultiBufferSource buffer, int light, int overlay
   ) {
      BlockState blockState = pressEntity.getBlockState();
      boolean isLit = (Boolean)blockState.getValue(MechanicalPressBlock.LIT);
      float pressPosition = pressEntity.getPressPosition(partialTicks, isLit);
      this.renderPress(matrixStack, buffer, light, overlay, SpecialModels.MECHANICAL_PRESS_PRESS.getModel(), pressPosition);
   }
   ...
   private void renderPress(PoseStack matrixStack, MultiBufferSource buffer, int light, int overlay, BakedModel model, float pressPosition) {
      if (model != null) {
         ...
         matrixStack.translate(0.0, (double)pressPosition, 0.0);
