...
public class ClientMeleeAttackHandler {
   public ClientMeleeAttackHandler() {
      super();
   }
   ...
   public static void spawnParticleEffect(Player player, LivingEntity target, ParticleType<?> particleType) {
      if (player.level().isClientSide) {
         ...
         clientLevel.addParticle((ParticleOptions)particleType, target.getX(), target.getY(), target.getZ(), 0.1, 0.1, 0.1);
      } else {
         ((ServerLevel)player.level())
            .sendParticles((SimpleParticleType)particleType, target.getX(), target.getY(), target.getZ(), 10, 0.5, 0.5, 0.5, 0.0);
      }
   }
}
