...
public class ThrowableNailBombEntity extends ThrowableGrenadeEntity {
   ...
   public ThrowableNailBombEntity(Level world, LivingEntity entity, int timeLeft) {
      super((EntityType<? extends ThrowableItemEntity>)ModEntities.THROWABLE_NAIL_BOMB.get(), world, entity);
   ...
   public void onDeath() {
      ...
      this.level().playSound(null, this.getX(), y, this.getZ(), SoundEvents.GENERIC_EXPLODE, SoundSource.BLOCKS, 1.0F, 1.0F);
   ...
   public static void createShrapnelExplosion(Entity entity, float radius, boolean damageBlocks) {
      if (!entity.level().isClientSide()) {
         ...
         world.playSound(
            null, origin.x, origin.y, origin.z, SoundEvents.GLASS_BREAK, SoundSource.NEUTRAL, 2.0F, 0.8F + world.random.nextFloat() * 0.4F
         );
         world.playSound(
            null, origin.x, origin.y, origin.z, SoundEvents.AMETHYST_CLUSTER_BREAK, SoundSource.NEUTRAL, 1.5F, 1.2F + world.random.nextFloat() * 0.3F
         );
         ...
         for (int i = 0; i < 40; i++) {
            ...
            serverLevel.sendParticles(ParticleTypes.CRIT, origin.x, origin.y, origin.z, 1, offsetX, offsetY, offsetZ, 0.02);
         }

         serverLevel.sendParticles(ParticleTypes.FLASH, origin.x, origin.y, origin.z, 1, 0.1, 0.1, 0.1, 0.0);
         int shrapnelCount = 30;
         float baseDamage = 14.0F;

         for (int i = 0; i < shrapnelCount; i++) {
            Vec3 direction = generateRandomDirection(world.random);
            Vec3 endPos = origin.add(direction.scale((double)radius));
   ...
   private static void traceShrapnelRay(Level world, Entity source, Vec3 start, Vec3 end, float damage, int rayIndex) {
      ...
      if (!hitSomething) {
         BlockHitResult blockHit = world.clip(new ClipContext(start, end, Block.COLLIDER, Fluid.NONE, source));
   ...
   private static List<Entity> findEntitiesAlongRay(Level world, Entity source, Vec3 start, Vec3 end) {
      ...
      return world.getEntities(source, searchBox, entity -> entity != null && entity.isPickable() && !entity.isSpectator() && isValidShrapnelTarget(entity, source));
   }
   ...
   private static void damageEntityWithShrapnel(Level world, Entity entity, Vec3 hitPos, float damage, Entity source) {
      ...
      if (entity instanceof LivingEntity livingEntity && !world.isClientSide()) {
         if (world.random.nextFloat() < 0.35F) {
            MobEffectInstance laceration = new MobEffectInstance(ModEffects.LACERATED, 200, 0, false, false, true);
   ...
   private void spawnExplosionParticles(Vec3 position) {
      if (!this.level().isClientSide) {
         ...
         for (int i = 0; i < 20; i++) {
            ...
            serverLevel.sendParticles(
               ParticleTypes.EXPLOSION, position.x + offsetX, position.y + offsetY, position.z + offsetZ, 1, speedX, speedY, speedZ, 0.1
            );
            serverLevel.sendParticles(
               ParticleTypes.CRIT,
               position.x + offsetX,
         ...
         for (int i = 0; i < 10; i++) {
            ...
            serverLevel.sendParticles(
               ParticleTypes.LARGE_SMOKE, position.x + offsetX, position.y + offsetY, position.z + offsetZ, 1, speedX, speedY, speedZ, 0.05
            );
         }
      }
   }
