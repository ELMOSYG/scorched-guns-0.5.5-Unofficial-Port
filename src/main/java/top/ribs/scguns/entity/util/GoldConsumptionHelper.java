...
public class GoldConsumptionHelper {
   ...
   public static boolean isGoldItem(ItemStack stack) {
      if (isPoisonItem(stack)) {
         ...
         return isVanillaGoldItem(stack) ? true : stack.is(ItemTags.PIGLIN_LOVED);
      }
   }
   ...
   public static boolean isPoisonItem(ItemStack stack) {
      return stack.is((Item)ModItems.SULFUR_CHUNK.get())
         || stack.is(((Block)ModBlocks.SULFUR_BLOCK.get()).asItem())
         || stack.is(((Block)ModBlocks.NETHER_SULFUR_ORE.get()).asItem())
         || stack.is(((Block)ModBlocks.SULFUR_ORE.get()).asItem())
         || stack.is(((Block)ModBlocks.DEEPSLATE_SULFUR_ORE.get()).asItem())
         || stack.is((Item)ModItems.SULFUR_DUST.get());
   }
   ...
   public static float getGoldNuggetValue(ItemStack stack) {
      if (stack.is(Items.GOLD_NUGGET)) {
         ...
         return 6.0F;
      } else if (stack.is(Items.RAW_GOLD_BLOCK)) {
         return 54.0F;
         ...
         return 6.0F;
      } else if (stack.is(Items.GOLDEN_HORSE_ARMOR)) {
         return 24.0F;
      } else if (stack.is(Items.NETHER_GOLD_ORE) || stack.is(Items.GOLD_ORE) || stack.is(Items.DEEPSLATE_GOLD_ORE)) {
         return 5.0F;
      } else if (stack.is(Items.GOLDEN_HELMET)) {
         return 30.0F;
      } else if (stack.is(Items.GOLDEN_CHESTPLATE)) {
         return 48.0F;
      } else if (stack.is(Items.GOLDEN_LEGGINGS)) {
         return 42.0F;
      } else if (stack.is(Items.GOLDEN_BOOTS)) {
         return 24.0F;
      } else if (stack.is(Items.GOLDEN_SWORD)) {
         return 12.0F;
      } else if (stack.is(Items.GOLDEN_PICKAXE)) {
         return 18.0F;
      } else if (stack.is(Items.GOLDEN_AXE)) {
         return 18.0F;
      } else if (stack.is(Items.GOLDEN_SHOVEL)) {
         return 6.0F;
      } else if (stack.is(Items.GOLDEN_HOE)) {
         return 12.0F;
      } else {
         return stack.is(ItemTags.PIGLIN_LOVED) ? 4.0F : 3.0F;
      }
   }
   ...
   public static void applyPoisonEffects(Mob entity, ItemStack poisonStack) {
      ...
      if (!entity.level().isClientSide && entity.level() instanceof ServerLevel serverLevel) {
         for (int i = 0; i < 8; i++) {
            ...
            double particleY = entity.getY() + (double)entity.getEyeHeight() - 0.1;
            ...
            serverLevel.sendParticles(ParticleTypes.ITEM_SLIME, particleX, particleY, particleZ, 1, velocityX, velocityY, velocityZ, 0.0);
         }
      }
   }
   ...
   public static void showSlagProductionParticles(Mob entity) {
      if (!entity.level().isClientSide && entity.level() instanceof ServerLevel serverLevel) {
         for (int i = 0; i < 5; i++) {
            ...
            double particleY = entity.getY() + (double)entity.getEyeHeight();
