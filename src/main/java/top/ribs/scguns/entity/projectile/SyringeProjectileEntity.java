...
public class SyringeProjectileEntity extends ProjectileEntity {
   ...
   public SyringeProjectileEntity(EntityType<? extends Entity> entityType, Level worldIn, LivingEntity shooter, ItemStack weapon, GunItem item, Gun modifiedGun) {
      ...
      this.canBeReused = this.random.nextFloat() < 0.25F;
   }
   ...
   protected void onHitEntity(Entity entity, Vec3 hitVec, Vec3 startVec, Vec3 endVec, boolean headshot) {
      ...
      if (!this.level().isClientSide) {
         ...
         if (effectLocation != null) {
            ...
            if (effect != null) {
               new MobEffectInstance(top.ribs.scguns.util.ScEffects.holder(effect), this.getProjectile().getImpactEffectDuration(), this.getProjectile().getImpactEffectAmplifier());
               int potionColor = effect.getColor();
               ((ServerLevel)this.level()).sendParticles(ColorParticleOption.create(ParticleTypes.ENTITY_EFFECT, potionColor), hitVec.x, hitVec.y + 0.5, hitVec.z, 1, 0.3, 0.3, 0.3, 0.0);
      ...
      if (!this.level().isClientSide) {
         ...
         if (effectLocation != null) {
            ...
            if (effect != null) {
               int potionColor = effect.getColor();
               this.level().levelEvent(2002, this.blockPosition(), potionColor);
               ((ServerLevel)this.level()).sendParticles(ColorParticleOption.create(ParticleTypes.ENTITY_EFFECT, potionColor), x, y, z, 1, 0.3, 0.3, 0.3, 0.0);
            }
         }
      }
   }
