...
public class FrogDartProjectileEntity extends ProjectileEntity {
   ...
   public FrogDartProjectileEntity(
      ...
      if (this.isInWater()) {
         ...
         this.setDeltaMovement(motion.scale(1.5));
   ...
   public void tick() {
      ...
      if (currentlyUnderwater && !this.wasUnderwater) {
         Vec3 motion = this.getDeltaMovement();
         this.setDeltaMovement(motion.scale(2.5));
      } else if (!currentlyUnderwater && this.wasUnderwater) {
         Vec3 motion = this.getDeltaMovement();
         this.setDeltaMovement(motion.scale(0.39999999999999997));
      }
   ...
   public double getModifiedGravity() {
      return this.isInWater() ? super.getModifiedGravity() * 0.1 : super.getModifiedGravity() * 3.0;
   }
