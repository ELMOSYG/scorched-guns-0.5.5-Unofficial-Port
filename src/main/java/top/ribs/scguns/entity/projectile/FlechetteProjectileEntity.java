...
public class FlechetteProjectileEntity extends ProjectileEntity {
   ...
   public FlechetteProjectileEntity(
   ...
   protected void onHitBlock(BlockState state, BlockPos pos, Direction face, double x, double y, double z) {
      ...
      if (!this.level().isClientSide && this.random.nextFloat() < 0.35F && this.shooter instanceof Player) {
         ItemEntity bulletEntity = new ItemEntity(
            this.level(), this.getX(), this.getY(), this.getZ(), new ItemStack((ItemLike)ModItems.NEEDLE.get())
         );
         double bounceStrength = 0.1;
         bulletEntity.setDeltaMovement((this.random.nextDouble() - 0.5) * bounceStrength, 0.15, (this.random.nextDouble() - 0.5) * bounceStrength);
