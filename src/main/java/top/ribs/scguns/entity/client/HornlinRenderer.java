...
public class HornlinRenderer extends HumanoidMobRenderer<HornlinEntity, HornlinModel> {
   private static final ResourceLocation TEXTURE = ResourceLocation.fromNamespaceAndPath("scguns", "textures/entity/hornlin.png");

   public HornlinRenderer(Context context) {
      super(context, new HornlinModel(context.bakeLayer(ModModelLayers.HORNLIN_LAYER)), 0.4F);
      this.addLayer(new HornlinRenderer.HornlinFoodItemLayer(this, context.getItemInHandRenderer()));
      this.addLayer(
         new HornlinRenderer.ScaledArmorLayer(
   ...
   private static class HornlinFoodItemLayer extends RenderLayer<HornlinEntity, HornlinModel> {
      ...
      public HornlinFoodItemLayer(HumanoidMobRenderer<HornlinEntity, HornlinModel> renderer, ItemInHandRenderer itemInHandRenderer) {
         super(renderer);
      ...
      public void render(
         ...
         if (entity.isEatingGold() || entity.isPreparingToEat()) {
            ...
            if (!heldFood.isEmpty()) {
               poseStack.pushPose();
               HornlinModel model = (HornlinModel)this.getParentModel();
   ...
   private static class ScaledArmorLayer<T extends HornlinEntity, M extends HumanoidModel<T>> extends HumanoidArmorLayer<T, M, HumanoidModel<T>> {
