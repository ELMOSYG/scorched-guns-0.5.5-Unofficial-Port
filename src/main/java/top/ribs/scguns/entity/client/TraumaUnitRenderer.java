...
public class TraumaUnitRenderer extends MobRenderer<TraumaUnitEntity, TraumaUnitModel<TraumaUnitEntity>> {
   public TraumaUnitRenderer(Context pContext) {
      super(pContext, new TraumaUnitModel(pContext.bakeLayer(ModModelLayers.TRAUMA_UNIT_LAYER)), 0.7F);
      this.addLayer(new TraumaUnitRenderer.TraumaUnitRedOverlayLayer(this));
   }

   public ResourceLocation getTextureLocation(TraumaUnitEntity pEntity) {
      return ResourceLocation.fromNamespaceAndPath("scguns", "textures/entity/trauma_unit.png");
   }
   ...
   static class TraumaUnitRedOverlayLayer extends RenderLayer<TraumaUnitEntity, TraumaUnitModel<TraumaUnitEntity>> {
      private static final ResourceLocation TEXTURE = ResourceLocation.fromNamespaceAndPath("scguns", "textures/entity/trauma_unit.png");
      ...
      public void render(
         ...
         if (entity.isPrimed()) {
            ...
            if ((int)(swell * 10.0F) % 2 != 0 && swell > 0.01F) {
               ...
               ((TraumaUnitModel)this.getParentModel())
                  .renderToBuffer(
                     poseStack, vertexConsumer, packedLight, LivingEntityRenderer.getOverlayCoords(entity, 0.0F), FastColor.ARGB32.colorFromFloat(0.8F, 1.0F, redIntensity * 0.5F, redIntensity * 0.5F)
                  );
            }
         }
      }
