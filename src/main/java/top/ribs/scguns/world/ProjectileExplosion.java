...
public abstract class ProjectileExplosion extends Explosion {
   ...
   /** The damage source handed to {@code super}; 1.21 removed the {@code Explosion#getDamageSource} accessor. */
   ...
   public void explode() {
      ...
      int minX = Mth.floor(this.x - (double)radius - 1.0);
      int maxX = Mth.floor(this.x + (double)radius + 1.0);
      int minY = Mth.floor(this.y - (double)radius - 1.0);
      int maxY = Mth.floor(this.y + (double)radius + 1.0);
      int minZ = Mth.floor(this.z - (double)radius - 1.0);
      int maxZ = Mth.floor(this.z + (double)radius + 1.0);
      ...
      EventHooks.onExplosionDetonate(this.world, this, entities, (double)radius);
      Vec3 explosionPos = new Vec3(this.x, this.y, this.z);

      for (Entity entity : entities) {
         if (!entity.ignoreExplosion(this)) {
            double strength = Math.sqrt(entity.distanceToSqr(explosionPos)) / (double)radius;
            if (!(strength > 1.0)) {
               ...
               entity.hurt(this.damageSource, (float)((int)((damage * damage + damage) / 2.0 * 7.0 * (double)radius + 1.0)));
               double blastDamage = damage;
               if (entity instanceof LivingEntity) {
                  blastDamage = getExplosionKnockbackAfterDampener((LivingEntity)entity, damage);
               }
   ...
   /**
    * Vanilla 1.20.1 {@code ProtectionEnchantment#getExplosionKnockbackAfterDampener}, which 1.21 removed when
    * enchantments became datapack entries.
    */
   private static double getExplosionKnockbackAfterDampener(LivingEntity entity, double damage) {
      int blastProtection = ScEnchants.level(entity, Enchantments.BLAST_PROTECTION);
      if (blastProtection > 0) {
         damage *= Mth.clamp(1.0 - (double)blastProtection * 0.15, 0.0, 1.0);
      }

      return damage;
   }

   protected abstract float getEntityDamageAmount(Entity var1, double var2);
}
