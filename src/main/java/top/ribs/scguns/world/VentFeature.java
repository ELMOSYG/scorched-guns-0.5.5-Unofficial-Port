...
public class VentFeature extends Feature<VentFeatureConfiguration> {
   ...
   public boolean place(FeaturePlaceContext<VentFeatureConfiguration> context) {
      ...
      VentFeatureConfiguration config = (VentFeatureConfiguration)context.config();
      ...
      if (config.shouldPlaceBaseBlock()) {
         ...
         for (Block requiredBlock : config.requiredBelowBlocks()) {
            if (belowBlockState.is(requiredBlock) && world.getBlockState(magmaPos.below()).isFaceSturdy(world, magmaPos.below(), Direction.UP)) {
               canPlaceBase = true;
               break;
            }
         }
      ...
      if (config.shouldCheckActiveLimit()) {
         canBeActive = this.countActiveVentsNearby(world, pos, config.checkRadius(), config.maxActiveNearby(), config.getVentBlock())
            < config.maxActiveNearby();
      }
      ...
      for (int i = 0; i < height; i++) {
         ...
         BlockState state = (BlockState)((BlockState)config.getVentBlock().defaultBlockState().setValue(VentBlock.VENT_TYPE, ventType))
            .setValue(VentBlock.ACTIVE, canBeActive);
         if (config.requiresWaterlogged() && state.hasProperty(GeothermalVentBlock.WATERLOGGED)) {
            state = (BlockState)state.setValue(GeothermalVentBlock.WATERLOGGED, true);
         }
   ...
   private int countActiveVentsNearby(LevelAccessor level, BlockPos pos, int checkRadius, int maxActive, Block ventBlock) {
      ...
      for (BlockPos checkPos : BlockPos.betweenClosed(pos.offset(-checkRadius, -checkRadius, -checkRadius), pos.offset(checkRadius, checkRadius, checkRadius))) {
         if (!checkPos.equals(pos)) {
            BlockState state = level.getBlockState(checkPos);
            if (state.is(ventBlock) && (Boolean)state.getValue(VentBlock.ACTIVE)) {
               activeCount++;
            }
