...
## 82.6 玩家验收提出的两条：持枪率 100% + 射速不正常（都已修 + 实测）

玩家反馈 ✓：**"警卫的持枪率是 100%"** ✓、**"警卫的枪械射速不正常"** ✓。两条都是真的 ✓，原因如下 ✓。

### 82.6.1 持枪率：`spawn_chance` 当时被照抄成 1.0
新版上游那份 JSON 里警卫就是 `1.0` ✓（本轮直接抄了过来 ✓）⇒ 人手一把 ✓。
现在改成 **0.25** ✓（= 玩家自己那份 1.20.1 兼容的 25% ✓），并且**每只警卫只抽一次签** ✓（§82.2 的记忆 ✓）。
**实测（6 次开服、共 240 只警卫）** ✓：8/40 ✓、9/40 ✓、10/40 ✓、6/40 ✓、14/40 ✓、10/40 ✓
⇒ **57/240 = 23.8%** ✓（240 样本下与 25% 相差不到 1 个标准差 ✓）。单次 40 只时 15%–35% 都属正常波动 ✓。

### 82.6.2 射速：直接用了 `rate` 当"两发间隔"，等于把半自动当全自动
`gunner_mobs.json` 里那 8 把枪的 `fireMode` **全是 `semi_automatic`** ✓，而 `rate` 是**扳机间隔** ✓：

| 枪 | rate（刻） | 弹匣 | 换弹（刻） | 旧实现 ⇒ 实际射速 |
|---|---|---|---|---|
| `scguns:callwell` | **2** | 2 | 30 | **每 2 刻一发 = 每秒 10 发** ✗ |
| `scguns:winnie` | 11 | 8 | 22 | 每 11 刻一发 ✓ |
| `scguns:pax` | 9 | 6 | 18 | 每 9 刻一发 ✓ |
| `scguns:saketini_ironport` | 27 | 6 | 70 | 每 27 刻一发 ✓ |

而且旧实现**绕过了 mob 自己的节奏机制** ✓：不看 `mobFireRateMultiplier` ✓（服务器把枪手调慢，警卫完全不受影响 ✗）、
没有**点射/间歇** ✓（`GunAttackGoal` 是"打 1–2 发、歇 40–80 刻" ✓，警卫却是持续连射 ✗）、
换弹还被**夹到 10–40 刻** ✓（慢枪换弹比同行快 ✓、快枪反而慢 ✓）。
现在**整套节奏照 `GunAttackGoal`** ✓：间隔 = `rate × mobFireRateMultiplier` ✓、
点射数 = `1 + rand(2 + ai_difficulty/2)` ✓、间歇 = `40+rand(40)` 刻 × `mobBurstDelayMultiplier` ✓、
换弹 = 该枪自己的 `reloadTimer` ✓；`ai_difficulty` 也从 JSON 传进目标 ✓（警卫是 1 ✓）。

**实测（1 只警卫 + 一只 4000 血、NoAI 的掠夺者，1200 刻）** ✓：
```
枪 scguns:pax（rate=9）     射击刻: 97 108 193 205 269 ⇒ 间隔 11 85 12 64
                             点射内 11–12 刻（≈ rate + 抖动）✓ 点射间 64–85 刻（= 间歇）✓
枪 scguns:winnie_millend（rate=14）  间隔 15 ✓（≈ rate）✓
目标状态线: attack=14 bursts=0 interval=6 reset=55 reload=false ammo=6 dist=8.0 see=true ally=false
```
* 也就是说：**点射 1–2 发、每发间隔≈该枪 rate、两轮之间歇 2–4 秒、打空后按该枪自己的换弹时间补弹** ✓
